...
public final class TransCoding {  
     ...
     * @return char字符 
     */  
    public static final char bytesToChar(byte[] b) {  
    ...
    /**
     * @method 将一个short短整型（2个字节），转换为byte[]
     * 
     * @param c
     * 
     */
	...
	public static final String toBinaryString(int i) {
        ...
        short charPos = 32;
        int radix = (1 << shift);//0010
        int mask = (radix - 1);//0001
        do {
            buf[--charPos] = digits[i & mask];
            i >>>= shift;
        } while (i != 0);

        return new String(buf, 16, 16);
    }
